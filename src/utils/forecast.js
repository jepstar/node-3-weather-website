...
const forecast = (latitude, longitude, callback) => {
    ...
    request({ url: urlForc, json: true }, (error, response) => {
      if (error) {
          ...
          callback(undefined, response.body.current.weather_descriptions[0] + '. Its is currently ' + response.body.current.temperature + ', but it feels like ' +  response.body.current.feelslike)
      }
    })
  } 
